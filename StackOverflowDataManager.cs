...
namespace StackMonitor {
  ...
  class StackOverflowDataManager {
    ...
    public List<Reputation> GetReputationChanges(int userid, long lastreputationchecktime) {
      string jsonData = RequestData(@"http://api.stackoverflow.com/1.0/users/" + userid + @"/reputation?key=xyaFp-6X1Umk-YpXHdruDA&fromdate=" + lastreputationchecktime);
      if (string.IsNullOrEmpty(jsonData)) {
        return null;
      }
    ...
    private List<Question> GetQuestionsNewerThanQuestionId(string jsonData) {
      ...
      var jobject = Newtonsoft.Json.Linq.JObject.Parse(jsonData);
      var questions = jobject.SelectToken("questions");
      foreach (var question in questions) {
        ...
        newquestion.URL = (string)question.SelectToken("question_answers_url");
    ...
    private List<Reputation> ParseReputationChanges(string jsonData) {
      ...
      var jobject = Newtonsoft.Json.Linq.JObject.Parse(jsonData);
      var repchanges = jobject.SelectToken("rep_changes");
      foreach (var repchange in repchanges) {
        ...
        newreputation.PositiveRep = (int)repchange.SelectToken("positive_rep");
        newreputation.NegativeRep = (int)repchange.SelectToken("negative_rep");
    ...
    public User PopulateUserInformation(int userid, int imagesize) {
      ...
      string jsonData = RequestData(@"http://api.stackoverflow.com/1.0/users/" + userid);
      if (string.IsNullOrEmpty(jsonData)) { return null; }
      var jobject = Newtonsoft.Json.Linq.JObject.Parse(jsonData);
      if ((int)jobject.SelectToken("total") == 0) { return null; }
      user.Name = (string)jobject.SelectToken("users[0].display_name");
      user.Reputation = (int)jobject.SelectToken("users[0].reputation");
      user.Badges.Add(new BadgeGroup() { Rank = BadgeRank.Gold, Count = (int)jobject.SelectToken("users[0].badge_counts.gold") });
      user.Badges.Add(new BadgeGroup() { Rank = BadgeRank.Silver, Count = (int)jobject.SelectToken("users[0].badge_counts.silver") });
      user.Badges.Add(new BadgeGroup() { Rank = BadgeRank.Bronze, Count = (int)jobject.SelectToken("users[0].badge_counts.bronze") });
      user.Image = DownloadUserImage((string)jobject.SelectToken("users[0].email_hash"), imagesize);
      return user;
    }
