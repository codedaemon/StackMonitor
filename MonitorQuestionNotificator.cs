...
namespace StackMonitor
{
  ...
  public partial class MonitorQuestionNotificator : Form
  {
    ...
    private void GenerateFormImage(Question question)
    {
      ...
      _formImage = new Bitmap(300, 150);
      using (Graphics g = Graphics.FromImage(_formImage))
      using (GraphicsPath path = HelperClass.GenerateRoundedRectPath(new Rectangle(0, 0, 300, 150), 5, HelperClass.RectangleCorners.All))
      using (GraphicsPath userPath = HelperClass.GenerateRoundedRectPath(new Rectangle(0, 101, 300, 48), 5, HelperClass.RectangleCorners.BottomLeft | HelperClass.RectangleCorners.BottomRight))
      ...
      using (Font arial11 = new Font("Arial", 11f))
      {
        ...
        g.DrawString(question.Title, arial11, Brushes.White, new Rectangle(11, 13, 280, 35));
        g.DrawString(question.Title, arial11, Brushes.Black, new Rectangle(10, 12, 280, 35));
        g.DrawString(question.Body, arial8, Brushes.White, new Rectangle(11, 46, 280, 35));
        g.DrawString(question.Body, arial8, Brushes.Black, new Rectangle(10, 45, 280, 35));
        string tagstring = string.Empty;
        question.Tags.ForEach(t => tagstring += t + " | ");
        tagstring = tagstring.Remove(tagstring.Length - 3);
        g.DrawLine(Pens.White, new Point(11, 84), new Point(Width - 9, 84));
        g.DrawLine(Pens.Black, new Point(10, 83), new Point(Width - 10, 83));
        g.DrawString(tagstring, arial10, Brushes.White, new PointF(11, 85));
        g.DrawString(tagstring, arial10, Brushes.Black, new PointF(10, 84));
        g.FillPath(userPathBrush, userPath);
        g.DrawPath(Pens.Gray, userPath);
        g.DrawString(question.Owner.Name, arial12, Brushes.DarkBlue, new Rectangle(50, 107, 245, 20));
        g.DrawString(question.Owner.ReputationString, arial12, Brushes.Black, new Rectangle(50, 125, 80, 20), centerformat);
        int xposition = 135;
        foreach (BadgeGroup badge in question.Owner.Badges.Where(b => b.Count > 0))
        {
          g.FillEllipse(StackMonitorClient.BadgeBrushes[badge.Rank], new Rectangle(xposition, 128, 10, 10));
          g.DrawEllipse(Pens.Black, new Rectangle(xposition, 128, 10, 10));
          g.DrawString(badge.Count.ToString(), arial12, Brushes.Black, new Rectangle(xposition + 10, 125, 35, 20), centerformat);
          xposition += 40;
        }
        if (question.Owner.Image != null) g.DrawImage(question.Owner.Image, new Rectangle(10, 109, 32, 32));
      }
    }
